...
export function initCursors({ reduced }, override) {
    ...
    return {
      update() {
        ...
        if (hot) {
          ...
          const p = 7
          targets = [
            { x: r.left - p, y: r.top - p }, { x: r.right + p, y: r.top - p },
            { x: r.left - p, y: r.bottom + p }, { x: r.right + p, y: r.bottom + p }
          ]
        } else {
          const s = 13
          targets = [
  ...
  function buildPen() {
    ...
    return {
      update() {
        ...
        root.style.transform = `translate(${px}px, ${py}px) rotate(${ang}rad)`
  ...
  function buildOrb() {
    ...
    return {
      update() {
        ...
        orb.style.transform = `translate(${px}px, ${py}px) rotate(${ang}rad) scale(${sx}, ${sy})`
        const isHot = !!hot
  ...
  const BUILDERS = { ring: buildRing, frame: buildFrame, pen: buildPen, orb: buildOrb }
